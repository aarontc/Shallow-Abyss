...
void AddMusic::on_txtSearch_textChanged(QString text)
{
	...
	m_query->clear();
	m_query->prepare("SELECT artists.artistid, artists.name AS artist_name, songs.songid, songs.title AS song_title FROM songs JOIN artists ON artists.artistid=songs.artistid WHERE LOWER(artists.name) LIKE LOWER(?) OR LOWER(songs.title) LIKE LOWER(?) ORDER BY artists.name ASC, songs.title ASC LIMIT ?");
	...
	while ( m_query->next() ) {
		...
		m_songIDs[row] = m_query->value(2).toULongLong();
